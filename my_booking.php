...
<h2>Your Bookings</h2>
<?php
    $query1= "SELECT * FROM bookings WHERE User_Name='abcd' and Canceled='No';"; 
    $query_run=(mysqli_query($conn,$query1));
if ($query_run->num_rows > 0) {
          ?>
  <table class="table">
       ...
       while($result=mysqli_fetch_array($query_run)){
           ...
           <td class="row-data1"><?php echo $result['City'] ?> </td>
           <td class="row-data1"><?php echo $result['Adult'] ?></td>
           <td class="row-data1"> <?php echo $result['Children'] ?></td>
           <td class="row-data1"> <?php echo $result['Room'] ?></td>
           <td class="row-data1"> <?php echo $result['Room Type']?></td>
           <td class="row-data1"> <?php echo $result['Bed'] ?></td>
           <td class="row-data1"> <?php echo $result['Check-in'] ?></td>
           <!-- <td class="row-data1"> </td> -->
           <td class="row-data1"> <?php echo $result['Stay'] ?> Nights</td>
           <td class="row-data1"> <?php echo $result['Bill Due'] ?></td>
           <td><input type="button" id="b2" value="Cancel" onclick="canc_booking('<?php echo $result['Booking_ID'] ?>')"></td>
...
<!-- Code end-->
    ...
    <script>
      function canc_booking(id){
        fetch('http://localhost/GoTravel-main/booking_cancel_ajax.php?id='+id)
                    .then(response => response.json())
                    .then(json => {
                        alert("Booking "+ JSON.stringify(json) + "is canceled");
                        ...
                        location.assign('my_booking.php');
                    })
